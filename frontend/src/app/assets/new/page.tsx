    ...
    <DashboardLayout>
      <div className="max-w-5xl mx-auto space-y-12 pb-24">
        ...
        {step === 1 ? (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-150">
            <div className="space-y-4">
              <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight leading-tight">
                Select <span className="text-indigo-600">Classification</span>
              </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {ASSET_TEMPLATES.map((tpl) => (
                <button
                  ...
                  className="group zen-card p-10 rounded-[3rem] hover:border-indigo-500/30 transition-all text-center flex flex-col items-center relative overflow-hidden"
                >
                  <div className="absolute top-0 right-0 w-32 h-32 bg-slate-50 blur-[40px] group-hover:bg-indigo-600/5 transition-colors" />
                  
                  <div className="w-24 h-24 bg-slate-100 rounded-[2rem] border border-slate-200 flex items-center justify-center text-5xl mb-8 shadow-inner group-hover:scale-110 group-hover:border-indigo-500/20 transition-all duration-500">
                    <span className="opacity-80 group-hover:opacity-100 transition-opacity">{tpl.icon}</span>
                  </div>
                  <h3 className="text-2xl font-black text-slate-900 mb-3 tracking-tight group-hover:text-indigo-600 transition-colors uppercase">{tpl.name}</h3>
                  ...
                  <div className="mt-auto w-full py-4 bg-slate-100 text-slate-400 font-black text-[10px] uppercase tracking-[0.2em] rounded-2xl group-hover:bg-indigo-600 group-hover:text-white transition-all">
                    Initiate Blueprint
                  </div>
                </button>
              ))}
            ...
            <div className="pt-16 border-t border-slate-100 mt-16 text-center">
              <h3 className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em] mb-10">Manual system configuration</h3>
              <div className="flex flex-wrap justify-center gap-6">
                {[
                  ...
                  <button
                    ...
                    className="flex items-center px-8 py-4 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-900 hover:border-slate-400 transition-all shadow-sm group"
                  >
          ...
          <div className="zen-card overflow-hidden animate-in fade-in zoom-in-95 duration-700 relative">
            <div className="p-10 md:p-16 relative z-10">
              <div className="flex items-center gap-8 mb-16">
                ...
                <div>
                  <h2 className="text-4xl font-black text-slate-900 tracking-tight uppercase leading-tight">Identifier <span className="text-indigo-600">Assignment</span></h2>
                  <p className="text-slate-400 text-xs font-black uppercase tracking-[0.2em] mt-2">Unit Classification: {formData.assetType}</p>
                </div>
              </div>

              {error && (
                <div className="mb-12 p-6 bg-rose-50 border border-rose-100 rounded-3xl flex items-start gap-5 text-rose-600 animate-in slide-in-from-top-4 duration-500">
              ...
              <form onSubmit={handleSubmit} className="space-y-12">
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-[2.5rem] p-8 md:p-12">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-6 mb-10">
                      <div className="flex items-center gap-4">
                        <div className="p-3 bg-white rounded-2xl text-indigo-600 border border-slate-100 shadow-sm">
                          <Check className="w-6 h-6" />
                        </div>
                        <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase">Protocol Bundling</h3>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer group">
                        ...
                        />
                        <div className="w-14 h-8 bg-slate-200 border border-slate-300 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[4px] after:left-[4px] after:bg-white after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-emerald-600 transition-colors"></div>
                        <span className="ml-4 text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-slate-600 transition-colors">Auto-Inject Protocols</span>
                      </label>
                    </div>
                    ...
                    {formData.includeDefaultTasks && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {selectedTemplate.defaultTasks.map(task => (
                          <div key={task.name} className="flex items-center bg-white border border-slate-100 px-5 py-4 rounded-2xl group/item hover:border-indigo-100 transition-colors">
                            <Calendar className="w-4 h-4 text-slate-300 mr-4 group-hover/item:text-indigo-400 transition-colors" />
                            <span className="text-xs font-bold text-slate-600 uppercase tracking-tight">{task.name}</span>
                            <span className="ml-auto text-[8px] font-black text-indigo-600 uppercase bg-indigo-50 px-2 py-1 rounded-lg border border-indigo-100">
                              {task.recurrenceType}
                            </span>
                          </div>
                        ))}
                ...
                <div className="pt-10 flex flex-col md:flex-row gap-6">
                  ...
                  <button
                    type="button"
                    onClick={() => setStep(1)}
                    className="btn-zen-secondary h-[72px] px-10"
                  >
